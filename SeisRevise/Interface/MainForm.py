...
class SpectrogramForm(QMainWindow, Ui_MainWindow):
    ...
    def spectrogram_calc(self):
        ...
        while True:
            ...
            # размер извлекаемого куска сигнала
            ...
            for file_path in bin_files_list:
                ...
                if signal is not None:
                    ...
                    self.teLog.append(
                        '\tВыборка файла {} успешно считана'.format(
                            bin_file_name))
                    QApplication.processEvents()
                else:
                    self.teLog.append(
                        '\tВыборка файла {} пуста. Обработка пропущена'.format(
                            bin_file_name))
                    QApplication.processEvents()
                    continue
                ...
                for component in components:
                    self.teLog.append(
                        '\t\tРасчет спектрограмм для компоненты {}...'.format(
                            component))
                    QApplication.processEvents()
